...
describe("Component Rendering", () => {
	...
	it("Renders correctly it´s loading state", () => {
		render(
			<Login handleSubmit={handleSubmitMock} handleRegisterClick={handleRegisterClickMock} closeErrorMessages={closeErrorMessagesMock} loginState={loginStateLoading} />
			, {wrapper: BrowserRouter}
		);
	...
	it("Renders correctly it´s loginError state", () => {
		render(
			<Login handleSubmit={handleSubmitMock} handleRegisterClick={handleRegisterClickMock} closeErrorMessages={closeErrorMessagesMock} loginState={loginStateLoginError} />
			, {wrapper: BrowserRouter}
		);
	...
	it("Renders correctly it´s networkError state", () => {
		render(
			<Login handleSubmit={handleSubmitMock} handleRegisterClick={handleRegisterClickMock} closeErrorMessages={closeErrorMessagesMock} loginState={loginStateNetError} />
			, {wrapper: BrowserRouter}
		);

		const networkErrorComponentHeader = screen.getByRole("heading", {level: 3, name: /Hubo un problema al intentar/i});
	...
	it("In the init state there are no elements of other states", () => {
		...
		const loadingComponentHeader = screen.queryByRole("heading", {level: 3, name: /esperando respuesta del servidor/i});
		const progresBar = screen.queryByRole("progressbar");
		const loginErrorParagraph = screen.queryByText( /Email \/ Clave Incorrectos/i );
		const networkErrorComponentHeader = screen.queryByRole("heading", {level: 3, name: /Hubo un problema al intentar/i});
